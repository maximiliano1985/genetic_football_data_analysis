...
def get_scores_previous_weeks(playing_stat,num):
    HomeScore_PrevW = []
    AwayScore_PrevW = []
    total_matches, nmatches = playing_stat.shape
    ...
    for i in range( total_matches ): 
        row = playing_stat.iloc[i]
        rows = playing_stat.iloc[:i]
        indx = -num # index used to go back in time
        
        prev_scores_home = rows[rows.HomeTeam == row.HomeTeam]
        index_matches, index_features = prev_scores_home.shape
        #print(row[['Date', 'HomeTeam']])
        #print(prev_scores_home[['Date', 'HomeTeam', 'HomeScores']])
        if index_matches >= 1:
            HomeScore_PrevW.append( prev_scores_home.iloc[indx].HomeScores )
            #print('Prev score ', prev_scores_home.iloc[indx].HomeScores)
        else:
            HomeScore_PrevW.append( np.nan )
            
        
        prev_scores_away = rows[rows.AwayTeam == row.AwayTeam]
        index_matches, index_features = prev_scores_away.shape
        if index_matches >= 1:
            AwayScore_PrevW.append( prev_scores_away.iloc[indx].AwayScores )
        else:
            AwayScore_PrevW.append( np.nan )
    
    col_h = 'HomeScore_PrevW%i'%num
    col_a = 'AwayScore_PrevW%i'%num
    playing_stat[col_h] = HomeScore_PrevW
    playing_stat[col_a] = AwayScore_PrevW
    return playing_stat
...
def get_mw(playing_stat):
    ...
    for i in range(total_matches):
        ...
        #mw = playing_stat.iloc[i].Date.isocalendar()[1]
        #print(playing_stat.iloc[i].Date, ' --> week: ', mw)
...
def fill_teams_pts(playing_stat):
    for index, row in playing_stat.iterrows():
        ...
        if np.isnan(row.HomeAttack):
            ...
            HomeAttack = np.mean( playing_stat[ playing_stat.HomeTeam == row.HomeTeam ].HomeAttack  )
            HomeMedium = np.mean( playing_stat[ playing_stat.HomeTeam == row.HomeTeam ].HomeMedium  )
            HomeDefense= np.mean( playing_stat[ playing_stat.HomeTeam == row.HomeTeam ].HomeDefense )
            HomeGK     = np.mean( playing_stat[ playing_stat.HomeTeam == row.HomeTeam ].HomeGK      )
            AwayAttack = np.mean( playing_stat[ playing_stat.AwayTeam == row.AwayTeam ].AwayAttack  )
            AwayMedium = np.mean( playing_stat[ playing_stat.AwayTeam == row.AwayTeam ].AwayMedium  )
            AwayDefense= np.mean( playing_stat[ playing_stat.AwayTeam == row.AwayTeam ].AwayDefense )
            AwayGK     = np.mean( playing_stat[ playing_stat.AwayTeam == row.AwayTeam ].AwayGK      )
            
            HomeScores = HomeAttack+HomeMedium+HomeDefense+HomeGK
            AwayScores = AwayAttack+AwayMedium+AwayDefense+AwayGK
            HomeAwayDifference = HomeScores-AwayScores
            
            playing_stat.set_value(index, 'HomeAttack' , HomeAttack )
            playing_stat.set_value(index, 'HomeMedium' , HomeMedium )
            playing_stat.set_value(index, 'HomeDefense', HomeDefense)
            playing_stat.set_value(index, 'HomeGK'     , HomeGK     )
            playing_stat.set_value(index, 'AwayAttack' , AwayAttack )
            playing_stat.set_value(index, 'AwayMedium' , AwayMedium )
            playing_stat.set_value(index, 'AwayDefense', AwayDefense)
            playing_stat.set_value(index, 'AwayGK'     , AwayGK     ) 
            
            playing_stat.set_value(index, 'HomeScores' , HomeScores ) 
            playing_stat.set_value(index, 'AwayScores' , AwayScores ) 
            playing_stat.set_value(index, 'HomeAwayDifference' , HomeAwayDifference ) 
        #print('____')
...
def  compute_team_scores(playing_stat, scores):
    ...
    for index, row in playing_stat.iterrows():
        ...
        if not match.empty:
            ...
            HS = float(match.HomeAttack)+float(match.HomeMedium)+float(match.HomeDefense)+float(match.HomeGK)
            AS = float(match.AwayAttack)+float(match.AwayMedium)+float(match.AwayDefense)+float(match.AwayGK)
            HomeScores.append( HS )
            AwayScores.append( AS )
            HomeAwayDifference.append( HS - AS )
        else:
            print('[WARNING] - whoscored.com has not match: ', row.Date, ' ', row.HomeTeam, ' vs ', row.AwayTeam)
            ...
            AwayGK.append( None )
            HomeScores.append( None )
            AwayScores.append( None )
            HomeAwayDifference.append( None )
    ...
    playing_stat['HomeScores'] = HomeScores 
    playing_stat['AwayScores'] = AwayScores 
    playing_stat['HomeAwayDifference'] = HomeAwayDifference
